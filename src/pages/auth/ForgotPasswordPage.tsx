import { Link } from 'react-router-dom';
import { useState } from 'react';
import { useAppOutlet } from '../../hooks/useAppOutlet';
import { Button } from '../../components/ui/Button';
import { Input } from '../../components/ui/Input';

export function ForgotPasswordPage() {
  const { app } = useAppOutlet();
  const [email, setEmail] = useState('ariana@optizenqor.com');
  const [message, setMessage] = useState<string | null>(null);

  return (
    <div>
      <p className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-400">Recovery</p>
      <h1 className="mt-3 text-3xl font-semibold text-slate-950 dark:text-white">Reset your password</h1>
      <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
        We’ll prepare reset instructions and route you back into your workspace.
      </p>

      <div className="mt-8 space-y-4">
        <Input label="Email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} />
        <Button
          className="w-full"
          onClick={async () => {
            const result = await app.sendResetLink(email);
            setMessage(result.message);
          }}
        >
          Send reset link
        </Button>
      </div>

      {message ?? app.authMessage ? (
        <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">{message ?? app.authMessage}</p>
      ) : null}

      <p className="mt-6 text-sm text-slate-500 dark:text-slate-400">
        Remembered it?{' '}
        <Link className="font-medium text-sky-600 dark:text-sky-300" to="/auth/login">
          Back to sign in
        </Link>
      </p>
    </div>
  );
}
