...
export function EventsPage() {
  ...
  async function handleCreateEvent() {
    ...
    try {
      ...
      setFeedback({ tone: 'success', message: 'Event created and refreshed from the backend.' });
    } catch (error) {
      setFeedback({
        tone: 'error',
        message: error instanceof Error ? error.message : 'Unable to create this event right now.',
      });
    } finally {
      setSubmitting(false);
    }
  ...
  async function handleRsvp(eventId: string) {
    ...
    try {
      ...
      setFeedback({ tone: 'success', message: 'Your RSVP was updated.' });
    } catch (error) {
      setFeedback({
        tone: 'error',
        message: error instanceof Error ? error.message : 'Unable to update your RSVP right now.',
      });
    } finally {
  ...
  async function handleSave(eventId: string) {
    ...
    try {
      ...
      setFeedback({ tone: 'success', message: 'Saved events were updated.' });
    } catch (error) {
      setFeedback({
        tone: 'error',
        message: error instanceof Error ? error.message : 'Unable to save this event right now.',
      });
    } finally {
      setEventActionId(null);
    }
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-semibold text-slate-950 dark:text-white">
            Meetups, workshops, and live gatherings
          </h1>
